...
# Define the token contract addresses for Optimism
...
class OptimismAdapter:
    def __init__(self):
        self.w3 = Web3(Web3.HTTPProvider(settings.BLOCKCHAINS['Optimism']['rpc']))
    ...
    def _detect_token_currency(self, tx):
        if tx['to'].lower() == USDT_CONTRACT.lower():
            ...
            return 'DAI'
        return 'ETH'  # Default is ETH (Optimism's native token)