...
# Define the token contract addresses for Fantom
...
class FantomAdapter:
    def __init__(self):
        self.w3 = Web3(Web3.HTTPProvider(settings.BLOCKCHAINS['Fantom']['rpc']))
    ...
    def _detect_token_currency(self, tx):
        if tx['to'].lower() == USDT_CONTRACT.lower():
            ...
            return 'DAI'
        return 'FTM'  # Default is FTM (Fantom's native token)