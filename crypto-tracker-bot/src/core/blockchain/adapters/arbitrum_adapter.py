...
# Define the token contract addresses for Arbitrum
...
class ArbitrumAdapter:
    def __init__(self):
        self.w3 = Web3(Web3.HTTPProvider(settings.BLOCKCHAINS['Arbitrum']['rpc']))
    ...
    def _detect_token_currency(self, tx):
        if tx['to'].lower() == USDT_CONTRACT.lower():
            ...
            return 'DAI'
        return 'ETH'  # Default is ETH (Arbitrum uses Ethereum's native token)