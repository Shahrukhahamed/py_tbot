...
# Define the token contract addresses for PulseChain
...
class PulseChainAdapter:
    def __init__(self):
        self.w3 = Web3(Web3.HTTPProvider(settings.BLOCKCHAINS['PulseChain']['rpc']))
    ...
    def _detect_token_currency(self, tx):
        if tx['to'].lower() == USDT_CONTRACT.lower():
            ...
            return 'DAI'
        return 'PLS'  # Default is PLS (PulseChain's native token)